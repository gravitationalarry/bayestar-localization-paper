...
\begin{equation}
	...
	\exp \left[ - \frac{1}{2}{\hat\rho_i}^2 - \frac{1}{2}{\rho_i}^2
		+ \hat\rho_i \rho_i \Re \left\{ e^{i \tilde{\gamma}_i} a^*(\tilde{\tau}_i)
		\right\}
	\right]
\end{equation}
%
with $\tilde{\gamma}_i = \hat\gamma_i - \gamma_i$, $\tilde{\tau}_i = \hat\tau_i - \tau_i$, and the template's autocorrelation function $a(t; \boldsymbol\theta_\mathrm{in})$ defined as
%
\begin{equation}
	a(t; \boldsymbol\theta_\mathrm{in}) := \frac{1}{\sigma_i(\boldsymbol\theta_\mathrm{in})} \int_0^\infty \frac{\left| h(\omega; \hat{\boldsymbol\theta}_\mathrm{in})\right|^2}{S_i(\omega)} e^{i \omega t} \,d\omega.
\end{equation}
...
\begin{equation}
	\mathcal{L}_\mathrm{BAYESTAR} = \exp \left[ - \frac{1}{2} \sum_i {\rho_i}^2 \right] I_0 \left[
		\left| \sum_i \hat\rho_i \rho_i e^{i \tilde{\gamma}_i} a^*(\tilde{\tau}_i)
		\right|
	\right],
\end{equation}
%
