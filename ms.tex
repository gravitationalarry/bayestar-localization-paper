...
Bayesian parameter estimation has many advantages, including broad generality and the ability to make probabilistically meaningful statements even with very low \ac{SNR} measurements. However, in problems of even modest complexity, the marginalization step burdens the analysis with nasty, many-dimensional, ill-behaved integrals. Almost all modern parameter estimation tactics are variations of \ac{MCMC} integration. Though powerful, \ac{MCMC} is slow and non-deterministic. With the most sophisticated \ac{CBC} parameter estimation codes, it still takes days to process a single event. This delay is undesirable for planning targeted \ac{EM} follow\nobreakdashes-up searches of \ac{LIGO} events.
...
The first task is to understand the uncertainty in the \ac{ML} point estimates from the detection. We begin with the \ac{CRLB}, which constraints the covariance of any unbiased estimator. The \ac{CRLB} has been widely applied in \ac{GW}. Here, we review the Fisher matrix calculation in \cite{fairhurst:2009} and discuss the interpretation.
