...
%
contain the integrals from Equations~(\ref{eq:fisher-autocor-gamma-gamma}, \ref{eq:fisher-autocor-tau-tau}, \ref{eq:fisher-autocor-gamma-tau}) and encode the departure of the autocorrelation likelihood from the likelihood of the full data at low SNR. All of the $\textproto{D}_{ij}(\rho)$ are sigmoid-type functions that run from 0 at $\rho = 0$ and asymptotically approach 1 as $\rho \rightarrow \infty$. The transitional SNR $\rho_\mathrm{crit}$ is largely the same for all three nontrivial matrix elements, and is determined by the time coincidence window $T$ and the signal bandwidth $\omega_\mathrm{rms}$.
%
\marginpar{The transitional SNR is probably proportional to some power of $(\omega_\mathrm{rms} T)$.}
