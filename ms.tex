...
\subsection{Detection, and matched filter bank as a maximum likelihood estimator}
...
The prevailing technique for detection of \acp{GW} from \acp{CBC} is to realize a \ac{MLE} from \Eref{eq:gaussian-likelihood} with the above signal model. This takes the form of a bank of matched filters, or template waveforms that are continually convolved with the incoming data stream. The matched filter or maximum likelihood estimates are given by
%
\begin{eqnarray}
