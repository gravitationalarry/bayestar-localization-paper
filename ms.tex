...
\end{eqnarray}
%
Matrix elements that are not listed have values that are implied by the symmetry of the Fisher matrix. Note that the minus signs are correct but a little confusing: despite them, $\mathcal{I}_{\gamma\gamma}, \mathcal{I}_{\tau\tau} \geq 0$ and $\mathcal{I}_{\gamma\tau} \leq 0$. The time integration limits $[-T, T]$ constitute a flat prior on arrival time, the time coincidence window between detectors.
...
\begin{itemize}
    ...
    \item Sampled PSD
    \item Waveform?
    \item Masses $\hat m_1, \hat m_2$
\end{itemize}

\subsection{Autocorrelation series}

\marginpar{This part may be too technical.}%
%
To calculate the autocorrelation sequence, we need to compute the inverse DFT of the autopower spectral density of the template. A BNS signal remains in the Advanced LIGO sensitive band for thousands of seconds, but we will only be interested in $\sim 10$\,ms of the autocorrelation sequence. If we have a sequence of length $N$, and we only need the first $M$ samples of the inverse DFT, where $M$ divides $N$ and $cM = N$, then we may make use of the following identity to compute the DFT with a speedup of $\log N/M$:
%
\begin{eqnarray*}
    x[n] &=& \sum_{k=0}^{N-1} \exp \left[ \frac{2 \pi i k n}{N} \right] Y[k] \\
    &=& \sum_{j=0}^{c-1} \sum_{m=0}^{M-1} \exp \left[ \frac{2 \pi i (m c + j) n}{c M} \right] Y[m c + j] \\
    &=& \sum_{j=0}^{c-1} \exp \left[ \frac{2 \pi i j n}{N} \right] \sum_{m=0}^{M-1} \exp \left[ \frac{2 \pi i m n}{M} \right] Y[m c + j].
\end{eqnarray*}
