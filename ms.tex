...
\end{equation}
%
\marginpar{Citations needed. Do we want to mention these omitted parameters?}%
%
Binary eccentricity is omitted as an intrinisc parameter because \acp{GW} circularize the orbit long before the inspiral enters \ac{LIGO}'s frequency range of $\sim$10\nobreakdashes--1000~kHz. Tidal deformabilities of the components are omitted because tidal effects have a negligible effect in the inspiral phase.

In this work we restrict ourselves to nonspinning sources, where $\mathbf{S}_1=\mathbf{S}_2=0$. Removing nuisance parameters requires either replacing them with their measured values or marginalizing over them. The chirp mass is typically well measured by the detection pipeline, so we use the maximum likelihood value, $\hat{\mathcal{M}}$, for it. We marginalize over the other variables using a uniform prior, i.e.
\begin{equation}
