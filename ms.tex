...
The marginalization over $D_\mathrm{L}$, $\cos \iota$, and $psi$ must be done numerically. The outermost integration is over $\cos \iota$ and $\psi$, and is done with Newton-Cotes by evaluating $\mathcal{L}_\mathrm{SNR}$ on a regular lattice in $\cos \iota$ and $\psi$. This integration scheme is first order in $\cos \iota$ and second order in $\psi$.
