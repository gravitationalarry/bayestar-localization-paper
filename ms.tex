...
Assuming circular orbits and no spin precession, we can write the \ac{GW} signal in each detector as a linear combination of two basis waveforms $H_0$ and $H_{\pi/2}$, such that the coefficients depend only on the extrinsic parameters $\boldsymbol\theta_\mathrm{ex}$ and the basis waveforms depend only on the intrinsic parameters $\boldsymbol\theta_\mathrm{in}$,
%
\begin{eqnarray}
	H_i(\omega; \boldsymbol\theta) &=& e^{-i\omega t_\oplus}\frac{D_{1,i}}{D_\mathrm{L}}
	\left(\begin{array}{cc}
		F_{+,i}(h_i, \delta) \\
		F_{\times,i}(h_i, \delta)
	...
	\end{array}\right) \\ &\cdot&
	\left(\begin{array}{cc}
		H_0(\omega; \boldsymbol\theta_\mathrm{in}) \\
		H_{\pi/2}(\omega; \boldsymbol\theta_\mathrm{in})
	\end{array}\right).
\end{eqnarray}
%
This linear combination matches the conventions used in \cite{PhysRevD.83.084002}, and is the frequency-domain equivalent of their Equation~(2.8). The quantities $F_{+,i}$ and $F_{\times,i}$ are the dimensionless antenna factors, and depend on detector $i$'s orientation and the declination and local hour angle $h_i$ of the source, and are derived in~\cite{PhysRevD.63.042003}.
