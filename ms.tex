...
\end{equation}
%
Then, we can write the signal model in a way that isolates all dependence on the extrinsic parameters $\boldsymbol\theta_\mathrm{ex}$ into the coefficients and all dependence on the intrinsic parameters $\boldsymbol\theta_\mathrm{in}$ into the basis waveform, by taking the Fourier transform of Equation~(2.8) of \cite{PhysRevD.83.084002}:
%
\begin{equation}\label{eq:full-signal-model}
    h_i(\omega; \boldsymbol\theta) = \e^{-i \omega t_\oplus}
    \frac{r_{1,i}}{r}
    \e^{2 i \phi_c}
    \left[
    ...
    \right]
    h(\omega; \boldsymbol\theta_\mathrm{in})
\end{equation}
%
...
\end{eqnarray}
%
A detection candidate consists of $\{\left\{ \hat\rho_i, \hat\gamma_i, \hat\tau_i \right\}_i, \hat{\boldsymbol\theta}_\mathrm{in}\}$. There are various ways to characterize the significance of a detection candidate, but in Gaussian noise the combined SNR $\hat\rho_\mathrm{c}$ suffices,
%
...
First, observe that at the true parameter values, $\hat{\boldsymbol\theta}_i = \boldsymbol\theta_i$, the logarithms of \Eref{eq:autocor-likelihood} and \Eref{eq:gaussian-likelihood-spa} have the same Jacobian. This is because the derivatives of the autocorrelation function are
%
\begin{equation*}
...
We can compute the Fisher matrix elements for the autocorrelation likelihood given by \Eref{eq:autocor-likelihood}, with detector subscript suppressed:
%
\begin{eqnarray}
    \nonumber
...
The detection pipeline supplies a candidate $\{\left\{ \hat\rho_i, \hat\gamma_i, \hat\tau_i \right\}_i, \hat{\boldsymbol\theta}_\mathrm{in}\}$, and discretely sampled noise \acp{PSD} $S_i(\omega_j)$ for all detectors. \marginpar{Discuss single-detector threshold here?}

We compute the GW signal for a source with intrinsic parameters equal to the detection pipeline's estimate, $h(\omega; \hat{\boldsymbol\theta}_\mathrm{in})$.
Then we find the SNR=1 horizon distance $r_{1,i}$ for each detector by numerically integrating \Eref{eq:horizon}.
...
\begin{equation*}
    p(r) \propto \cases{
        r^m & if $r_\mathrm{min} < r < r_\mathrm{max}$ \\
        0 & otherwise
    }
\end{equation*}
%
...
\subsubsection{Integral over distance: adaptive Gaussian quadrature method}
...
We found two efficient ways to evaluate the distance integral. The first method is to use adaptive Gaussian quadrature, but with an educated choice of initial subdivisions. The distance integral can be written as
%
\begin{eqnarray}\label{eq:distance-integral}
    \mathscr{F} &=&
...
%
\begin{eqnarray*}
    p^2 &=& \frac{1}{2} r^2 \sum_i {\rho_i}^2 \\
    b &=& r \left| \sum_i \hat\rho_i \rho_i e^{i \tilde{\gamma}_i}
        a_i^*(\tilde{\tau}_i) \right| \\
    \overline{I}_0(x) &=& \exp(-|x|) I_0(x).
\end{eqnarray*}
%
The coefficients $p^2$ and $b$ are nonnegative and independent of distance. The symbol $\overline{I}_0$ denotes an exponentially scaled Bessel function. In the limit of large argument, $I_0(|x|) \sim \exp(|x|) / \sqrt{2 \pi |x|}$ \cite{Olver:2010:NHMF}\footnote{\url{http://dlmf.nist.gov/10.40.E1}}. The scaled Bessel is useful for evaluation on a computer because it has a relatively small range $(0, 1]$ and varies slowly in proportion to $x^{1/2}$. If we neglect both the Bessel function and the $r^m$ prior, then the approximate likelihood $\exp(-p^2/r^2 + b/r)$ is maximized when
%
\begin{equation}\label{eq:ml-distance}
    r = r_0 = 2p^2/b.
\end{equation}
%
The likelihood takes on a factor $\eta$ (say, $\eta=0.01$) of its maximum value when
%
\begin{equation}
...
\subsubsection{Integral over distance: importance sampling method}

The second method of evaluating the distance integral, and the one used in practice, is similar to importance sampling. Let's say that we want to evaluate an integral
%
...
\end{equation}
%
If we apply the same change of variables to \Eref{eq:importance-sampling-difficult-integral}, we get
%
...
\end{equation}
%
If $f(x)$ and $g(x)$ have sufficiently similar behavior, then their quotient $f/g$ will be better behaved than $f$ itself, and more amenable to any given quadrature technique. When the transformed integral \Eref{eq:importance-sampling} is evaluated using Monte Carlo integration, this technique is referred to as importance sampling.

We use the same kind of change of variables with fixed\nobreakdashes-order Gaussian quadrature. We first find the approximate \ac{ML} distance $r_0$ using \Eref{eq:ml-distance}. If $r_0 < r_\mathrm{min}$ or $r_0 > r_\mathrm{max}$, then the ML peak lies outside of the limits of integration and the original integrand behaves like a low-order power law. In this case, we use 10\nobreakdashes-point Gaussian quadrature to evaluate \Eref{eq:distance-integral}.

If, on the other hand, $r_\mathrm{min} \leq r_0 \leq r_\mathrm{max}$, then the integrand contains a sharp peak that we can smooth out with importance sampling. We first re-scale the distance integral \Eref{eq:distance-integral} so that the peak value of the integrand is $\sim 1$:
%
\begin{equation}
...
Last, there are a few special cases where \Eref{eq:distance-integral} can be evaluated exactly. If $p^2=0$ (which implies $b = 0$), then the integral may be expressed in terms of logarithms. If $b = 0$ but $p^2 \neq 0$, then the integral may be expressed in terms of the exponential integral function,
%
\begin{equation}
...
We begin by evaluating the posterior probability density at the center of each of the $N_{\mathrm{pix},0} = 768$ pixels of an $N_{\mathrm{side},0}=8$ \ac{HEALPix} grid. At this resolution, each pixel has an area of 54\,deg$^2$. We then rank the pixels by contained probability (assuming constant probability density within a pixel), and subdivide the most probable $N_{\mathrm{pix},0}/4$ pixels into $N_{\mathrm{pix},0}$ new pixels. We then evaluate the posterior again at the pixels that we subdivided, sort again, and repeat eight times, so that we have evaluated the posterior probability density a total of $8 N_{\mathrm{pix},0}$ times. On \emph{most} iterations, we descend one level deeper in \ac{HEALPix} resolution. The resulting map is a tree structure that consists of a mix of several resolutions. We traverse the tree and flatten it into the highest resolution represented. The highest possible resolution is $N_\mathrm{side}=2^{11}$, with an area of 1.7\,deg$^2$ per pixel.\footnote{Although the resulting sky map contains $N_\mathrm{pix}=24\,576$ pixels, at most $4\,800$ pixels have distinct values. For the purpose of delivery to observers, therefore, the output is always \texttt{gzip}\nobreakdashes-compressed with a ratio of $\sim 5:1$.}
